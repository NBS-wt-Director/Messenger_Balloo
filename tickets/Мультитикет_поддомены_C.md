...
- ⏳ **Остаток тикета №1 (следующие сессии):** P34 — редизайн `ChatViewScreen` по `mockups/balloo-su/chats.html` (+`chats.md`; примечание: макет называется `chats.*`, не `chat.*`) — дизайн-система common.css (октагон-аватары, пузыри с угловыми срезами `--bubble-sender/receiver`, msg-ticks, реакции-chips, date-separator, slash-подсказки, PollWidget/VoiceMessage/EditHistory по чек-листу chats.md); P35 — единая Topbar/Footer через `@balloo/ui` на ВСЕХ экранах web (сейчас: auth-экраны имеют шапку-классы, MainLayout — инлайн topbar/footer, прочие экраны — по-разному); тема russian — темы синхронны с common.css (переменные идентичны, градиент флага есть), остаток = аудит экранов на непрозрачные фоны, закрывающие градиент (как P27 на auth); скриншот-сверка 3 тем; docker build на сервере.
...
**Статусы:** №0 ✅ ЗАВЕРШЁН (2026-09-20: код `00cfa37`→`9eeaf4b` в origin, **задеплоен на проде, smoke `PASS=8 FAIL=0 SKIP=0`**, шлюз `172.19.0.1` цел; остаток — панели OAuth + живой вход, действие владельца) · №1 🔄 IN PROGRESS (2026-09-21: P33-фикс выполнен в репо — `OAuthAccountInactiveError` в `oauthLogin` (2 точки: вход по привязке + привязка по email), колбэки → `oauth_error=account_inactive`, фронт-тексты 20 языков, тест 3/3, server 245/245 web 155/155 shared 66/66, tsc чисто; скрипт чистки `docker/prod/p33-clean-e2e-account.sh` ждёт решения владельца; **`@balloo/ui` создан, web перешёл на него без визуальных изменений** (tsc 0, vitest 155/155, vite build OK, Dockerfile.web обновлён), docs/01 обновлён; остаток: P34 редизайн чата по `mockups/balloo-su/chats.html`, P35 шапка/подвал на всех экранах, аудит темы russian, скриншот-сверка — следующие сессии) · №2 pending · №3 pending · №4 pending · №5 pending · №6 pending
...
**✅ P33-фикс задеплоен; ЧИСТКА ТЕСТОВОГО АККАУНТА НА ПРОДЕ НЕ ВЫПОЛНЕНА (2026-09-21, по выводу консоли сервера).**

- **Деплой (есть в выводе):** `pull 9acbd4d..bcf8643` → `build server web` (335s, оба образа Built) → `up -d server web` → `balloo-server Up (healthy)`, `balloo-web Up`, postgres/redis/minio healthy. P33-фикс (`3c7bd49`) и тикет №1 (`51537ca`) **на проде работают**.
- **Баг скрипта чистки (найден на проде):** `ERROR: column "provider_id" does not exist`, HINT: `oauth_accounts.providerId`. Причина: схема создана Prisma, колонки в PostgreSQL — `camelCase` и в кавычках, а скрипт писал `snake_case` (`user_id`, `provider_id`, `created_at`, …). В выводе сервера — три падения подряд (read-only, `--apply`, `--apply`), **успешного прогона нет**.
...
- ⚠️ **Остаток P33 (действие владельца на сервере):** `git pull --ff-only origin main` (забрать `7f14b25`) → `bash docker/prod/p33-clean-e2e-account.sh` (read-only, должны печататься привязки без ошибки) → `--apply` → дождаться `email=<NULL>`, `oauth_accounts=0`, `email ... занят: 0`. Только после этого вход через Яндекс создаст НОВЫЙ аккаунт владельца (сейчас он по-прежнему упрётся в deleted-аккаунт `cmu425bbl00004z5qab07sfiw` / `e2e_smtp_test`).
- ⚠️ **Исправление записи о коммите `7f14b25`.** Его сообщение («зачищал не тот email — columnP2 не существовал», про `emailP2`, refresh-токены, rc=3/rc=4) **не соответствует диффу**: столбца `columnP2`/`emailP2` в схеме нет и никогда не было, rc=3/rc=4 в скрипте нет. Дифф при этом верный — именно он проверялся на стенде. Настоящий баг: `snake_case` вместо Prisma-`camelCase`. Сообщение опубликовано (`origin/main`), история НЕ переписывалась (force-push сломал бы `git pull --ff-only` на сервере) — правда зафиксирована этим коммитом. Урок в реестр: перед `git commit` сверять `git diff` с текстом сообщения; не описывать в сообщении то, чего нет в диффе; не объявить действие выполненным без вывода команды, это это доказывающего.
...
- Работа (новая сессия, отдельный тикет внутри №1 или самостоятельный): переписать `packages/web/src/screens/chat/ChatViewScreen.tsx` **по макету `mockups/balloo-su/chat.html` (+ chat.md)** — структура (topbar/sidebar/список чатов/облако сообщений), дизайн-система `mockups/assets/common.css` (октагон-аватары, пузыри с угловыми срезами, темы), поведение. Сверка скриншотами по всем 3 темам.
- Это большая задача — только в новой сессии, по мультитикет-протоколу.
