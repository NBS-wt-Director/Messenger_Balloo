...
|---|---|---|---|
| 9 (эт. 22) | Рабочая гипотеза P11: релей через `smtp.mail.ru` (в env лежат `MAILRU_CLIENT_ID/SECRET/REDIRECT_URI`, ящик `noreply@balloo.su` с паролем) | **P11 решено пользователем (2026-09-16): SMTP поднимаем свой на сервере**, сторонние ящики не используем | Явное решение пользователя. Само решение по Mail.ru не было «неверным» — оно отменено владельцем. Следствие: для доставляемости нужно закрыть 3 условия (исходящий :25, rDNS, SPF) — их проверяет батч G; схема релея выбирается по результатам (контейнер-релей в compose предпочтительнее установки postfix на хост — сервер мультипроектный, правило 5 §0) |
...
2. **P11 SMTP — ✅ решено пользователем (2026-09-16): поднимаем свой SMTP на сервере** (журнал, строка 22; гипотеза Mail.ru-релея отменена владельцем). Перед установкой — батч G (read-only): исходящий :25, DNS (`MX`/`SPF`/`DMARC`), rDNS, существующий MTA, docker-сеть. Схема по результатам: предпочтительно контейнер-релей в compose (не postfix на хосте — мультипроектность). Режим TLS в коде задаётся портом (`465` = implicit TLS, иначе STARTTLS), `SMTP_TLS` код не читает.
...
| P11 | SMTP не настроен | `🟡` **решение принято пользователем 2026-09-16: свой SMTP на сервере** (журнал 22). Релея пока нет (`SMTP_HOST=localhost` изнутри контейнера = сам контейнер, `ECONNREFUSED 127.0.0.1:587`) | Батч G (read-only): :25 наружу, DNS, rDNS, MTA на хосте, docker-сеть → выбор схемы (контейнер-релей в compose предпочтительнее postfix на хосте) → установка → тест письма из контейнера |
