...
> Создан: 2026-09-12. Последнее обновление: **2026-09-17, этап 31** (хвост этапа 30 закрыт выводом: `restarts=0` после `--force-recreate`; попытка WS-теста пачкой §1.5 провалилась на вводе — `read` склеился при вставке, `login=400` «Email и пароль обязательны», прод ни при чём; пачка заменена скриптом `docker/prod/ws-positive-test.sh` (батч M, §11) — локально проверена на стенде с тем же `ws@8.21.1`: `101` подтверждён, `rc=28` после 101 — норма; next — прогон батча M, UptimeRobot, P7/P12, финал/ребут).
...
## 1. ТЕКУЩАЯ ТОЧКА (2026-09-17, этап 31 — хвост этапа 30 закрыт (`restarts=0`); WS-тест пачкой §1.5 провалился на вводе (`read` склеился → `login=400`), прод исправен; заменена скриптом `docker/prod/ws-positive-test.sh` (батч M); next — прогон батча M, UptimeRobot, P7/P12, финал/ребут)

### 1.0 Что делать в этой сессии прямо сейчас

1. **Разбор вывода консоли, присланного 2026-09-17 (этап 31):** (а) `restarts=0`, `startedAt=2026-09-17T04:41:29Z` — незакрытый замер этапа 30 (рестарты после `--force-recreate`) **закрыт**; (б) `select email from users` → `probe@balloo.su`, `o8eryuhtin@yandex.ru` — тестовый аккаунт этапа 27 на месте; (в) `login=400` — **первая строка пачки §1.5 не выполнилась**: при вставке многострочной пачки `read -rs -p 'email: ' E` склеился со следующей строкой, `E`/`P` остались пустыми → тело `{"email":"","password":""}` → `400 «Email и пароль обязательны»` (проверено по коду `authController.ts:102-108`: `400` бывает только при пустых полях; неверный пароль дал бы `401`). Все последующие `401`/`ws_token_len=0` — ожидаемые следствия пустых cookie. **Прод и nginx исправны, дефект был в форме батча** — ровно класс правил 14–16 §0. Пачка §1.5 аннулирована, заменена скриптом (батч M).
2. **Ротация секретов §8 НЕ выполняется** (решение пользователя 2026-09-17, без изменений): P6, P13, A2, `SMTP_PASSWORD`, sudo-пароль — остаются как есть осознанно. Не предлагать повтор.
3. **Текущее действие — батч M (§11):** на сервере после commit+push — `cd ~/balloo && git pull --ff-only origin main && bash docker/prod/ws-positive-test.sh` (одна команда; `read -rs` внутри скрипта; вводить email `o8eryuhtin@yandex.ru` и пароль аккаунта). Скрипт закрывает сразу: WS `101` (§10), P3 (upload 5 МБ без `413`), поведенческий признак `[WS] Connection from user`, контроль `restarts`. **Важно про признак:** curl после `101` держит WS-сессию до `--max-time` и завершается с `rc=28` — это норма (проверено локально на стенде с тем же `ws@8.21.1`: `ws_with_token=101`, `rc=28`, сервер логирует `WS_ACCEPTED`). Критерий — код `101`, не rc.
...
**Батч M — скрипт `docker/prod/ws-positive-test.sh`** (§11): после commit+push скрипта на сервере одной командой `cd ~/balloo && git pull --ff-only origin main && bash docker/prod/ws-positive-test.sh`. Вводить при промптах: email `o8eryuhtin@yandex.ru` (аккаунт этапа 27, подтверждён в БД выводом 2026-09-17), пароль — знаете только вы. Скрипт закрывает WS `101` (§10) + P3 (upload 5 МБ) + поведенческий признак `[WS] Connection from user` + контроль `restarts`. Дальше: UptimeRobot (пользователь, без сервера) → P7/P12 → финал с ребутом по вашей команде.

Выполненные батчи, не повторять: A (шаги 1–3), B, E, F, G, H, I, J, K, L. Батч C (read-only) остаётся полезным для P12/`alpha.balloo.su` — его можно выполнить одним заходом вместе с P7.
...
Файлы, созданные ассистентом в ходе этого тикета (список сверен с диском этапом 29, дополнен этапом 31): `docker/prod/nginx/balloo-docker.conf`, `docker/prod/rotate-secrets.mjs`, `docker/prod/apply-rotated-secrets.sh`, `docker/prod/enable-probes.sh`, `docker/prod/enable-probes.selftest.sh`, `docker/prod/smtp-relay.sh`, `docker/prod/ws-positive-test.sh` (этап 31, батч M), `packages/server/src/process-handlers.ts`, `packages/server/src/__tests__/process-handlers.test.ts`, `packages/server/src/__tests__/ws-handshake.test.ts`, этот документ.
...
### Батч M — позитивный WS-тест + P3 (upload 5 МБ) + контрольные замеры — ТЕКУЩЕЕ ДЕЙСТВИЕ (этап 31)
