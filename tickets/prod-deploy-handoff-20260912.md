...
2. **P11 SMTP — 🟡 батч G выполнен (этап 23), схема выбрана: приложение шлёт через хостовый postfix** (`SMTP_HOST=172.19.0.1:25`, docker-подсеть в `mynetworks`), новый контейнер-релей не нужен — на хосте уже стоят postfix+dovecot (enabled, `:25`), MX=`mail.balloo.su`, SPF уже разрешает наш IP, исходящий `:25` открыт. Следующий шаг — батч H (read-only, §11) + **подтверждение пользователя, что postfix наш** (правило 5 §0), затем правка `mynetworks` одним заходом → env → тест письма. Режим TLS в коде задаётся портом (25 → без TLS внутри docker-сети), `SMTP_TLS` код не читает.
