...
2. **P11 SMTP — 🟡 батчи G+H выполнены (этапы 23–24), схема: приложение шлёт через хостовый postfix** (он наш: `myhostname=mail.balloo.su`, `mydestination=balloo.su`, доставляет напрямую, `status=sent` в логе). Блокеры: `mynetworks` без docker-подсети + ufw без правила на 25 (отсюда `timeout` из контейнера). **Следующий шаг — батч I: `bash docker/prod/smtp-relay.sh`** (бэкап `main.cf` → `mynetworks` + `172.19.0.0/16` → `postfix check` с откатом → `reload` → `ufw allow from 172.19.0.0/16 to any port 25` → баннер 220 из контейнера). Затем батч J: `SMTP_HOST=172.19.0.1`/`SMTP_PORT=25` в env + пересоздание контейнера + тестовое письмо. Отдельный вопрос пользователю (не блокер): открыть ли 25 наружу для **входящей** почты @balloo.su (сейчас ufw её режет, MX указывает сюда).
