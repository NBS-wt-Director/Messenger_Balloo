...
> Создан: 2026-09-12. Последнее обновление: **2026-09-16, этап 20** (батч F выполнен успешно: probes закрыты basic-auth `401/200`, htpasswd перезаписан свежим паролем, autostart включён; next — UptimeRobot + `certbot renew --dry-run`).
...
3. **Следующие шаги (по §6):**
   - **UptimeRobot (пользователь, без сервера):** на сервере `cat ~/balloo-probe-credentials.txt` (файл `600`, в чат не переносить), в панели UptimeRobot — монитор типа «HTTP(s)», URL `https://api.balloo.su/health`, GET, 60 с, Basic Auth `uptimerobot` + пароль из файла. Признак работы: монитор зелёный, а логи не показывают постоянные 401.
   - **`sudo certbot renew --dry-run`** (одна команда на сервере, §6 шаг 8) — обязательна после правки nginx (критерий P5). Ждём `The dry run was successful`.
   - Дальше по порядку: P11 SMTP (решение пользователя), ротация секретов §8 (один заход), WS-тест `101` (§1.5), финал/ребут по команде пользователя.
