...
> Создан: 2026-09-12. Последнее обновление: **2026-09-15, этап 15** (вторая пачка PROBES+AUTOSTART выполнена с ошибкой ассистента, результат аннулирован → батч E).
...
## 1. ТЕКУЩАЯ ТОЧКА (2026-09-15, этап 16 — PROBES/AUTOSTART: батч E прочитан, выдуманный контекст аннулирован, следующий шаг — батч F)

### 1.0 Что делать в этой сессии прямо сейчас

1. Батч E выполнен (вывод — в журнале, строка 16). Фактическое состояние: живой конфиг **без** `auth_basic` вообще, `/etc/nginx/.htpasswd-balloo` существует, но **0 байт** (перетёрт `install … /dev/null` в 16.1 до `htpasswd: command not found`), `balloo.service` статус `enable` не подтверждён (команда `systemctl show` склеилась с `awk` и не выполнилась).
2. Следующий шаг — **батч F** (он же `docker/prod/enable-probes.sh`, коммитится этим же этапом): на сервере `cd ~/balloo && git pull --ff-only origin main`, затем **одной** строкой `bash ~/balloo/docker/prod/enable-probes.sh`. Внутри скрипта: `read -rs -p` (валидный синтаксис), хеш через `openssl passwd -6`, бэкап живого конфига, `cp` канона, `nginx -t` с откатом, один `reload`, `systemctl enable balloo.service`, проверки `PASS/FAIL` (строгие признаки: без пароля 401, с паролем 200).
3. **Аннулировано как выдумка предыдущих сессий (не возвращать):** эндпоинты `/livez`, `/readyz`, `/api/livez`; файл `/etc/nginx/.htpasswd-probe`; скрипт `fix-auth-probes.sh`; «пустой хеш в htpasswd — источник 401». Реальный `401` на `127.0.0.1:8080` даёт **чужой node** (батч B: `127.0.0.1:8080` — чужой), к нашему стеку отношения не имеет. Наши пробы — `/health`, `/health/ready` (§1.0.4, §3).
4. **Миграционный блок предыдущего сообщения — мой ложный дрейф, отозван.**Pending-миграций нет (этап 1 завершён: `tables=51`, «up to date»). Команды `docker compose exec …` были даны в неверном контексте (из `~/balloo` вместо `docker/prod` + `--env-file .env.production`) и умерли на `service "postgres" is not running`; файлов `/tmp/migrations.sql`, `./backup.sql` нет. **Данные не тронуты, откатывать нечего.**
5. **Ничего не выдумывать:** каждый путь, имя юнита и утилита в команде — либо из §3, либо из §1.1/§1.6/§1.8/журнала этого документа.

### 1.0.1 Собственные ошибки последних двух сессий (зафиксировано, чтобы не повторять)
...
**Последствие, требующее лечения (P14):** файл `/etc/nginx/.htpasswd-balloo` записан из `$PW`, которая не была заполнена → внутри, скорее всего, `uptimerobot:<hash пустой строки>`. Если канон когда-нибудь наложат без перезаписи файла, basic-auth станет обходимым пустым паролем. Лечится перезаписью файла в батче F (одна тема — одна команда, `nginx -t` + один `reload`).
...
| P14 | **Неизвестно, что лежит в `/etc/nginx/.htpasswd-balloo`, но задать пароль та пачка не могла.** Два реальных сообщения об ошибке в одном выводе: `sudo: htpasswd: command not found` и `-bash: read: «PW?пароль для uptimerobot: »: not a valid identifier` (форма `VAR?промпт` в bash не существует) → `PW` осталась пустой, и `openssl passwd -6` получил пустой stdin | `❓` содержимое файла не смотрели. Накладывать конфиг с `auth_basic` поверх неизвестного/пустого файла нельзя | Пересоздать файл одной строкой с корректным `read -rs -p` и `openssl passwd -6 -stdin`, затем проверить без вывода хеша: `awk -F: '{print "users="NR" hash_prefix="substr($2,1,3)" len="length($2)}'` → ждём `users=1 hash_prefix=$6$ len>90` |
| P15 | **Не подтверждено, что канон дошёл до живого конфига.** В выводе поздней пачки нет ни `cp`, ни `nginx -t`, ни `reload` — вместо этого промпт `cfr_balloo@aedgar:~/read -s 'PW?пароль: '`, то есть строки склеились при вставке. Косвенно: `api_без_пароля=200` → basic-auth не работает | `❓` живой конфиг не сверялся | `sha256sum /etc/nginx/sites-enabled/balloo-docker.conf` против репозиторного `e4a0f64d36062ccfa5a6ca8ec4990d08b909320addc4ff8a7074b7efd7fafca7` (реально измерено на ноутбуке в `docker/prod/nginx/balloo-docker.conf`, совпадает с `HEAD`) + `grep -c auth_basic_user_file` → ждём `2` |
...
`🟡` Вариант выбран и реализован в репозитории: `c32f81c` — общий `http`-блок с `map $uri $balloo_probes_needauth` и `location ~ ^/(status|metrics|health|health/ready)`, закрывающий `/health` и `/health/ready` на **оба** HTTPS-сервера (`balloo.su`, `api.balloo.su`). На сервере **не применено**: `nginx -T` даёт `auth_basic = 0`, публичные пробы отвечают `200` (§1.0.2 строка 2).
Два условия перед наложением: (а) реальный пароль в файле `htpasswd` (сейчас там пустой хэш — P14); (б) понимание, что это за 4 `auth_basic` уже есть в `nginx -T` (§1.6 строка 333) и не перекрывают ли они наши же локации (P15).
Файл паролей решено держать **вне** `/etc/nginx`: `/etc/balloo/probes/htpasswd`, `0640 root:www-data`, каталог `0750` (в `/etc/nginx/` конфиги читаются группой `adm`/всеми, а секретный файл в каталоге конфигов — плохая практика; `common.css`-подобная «единая точка» здесь — путь, зафиксированный в скрипте).
...
`⛔` Формально закрыт 2026-09-15 (`is-enabled = enabled`), но **вывод противоречив** и требует перепроверки: в той же пачке `systemctl is-active` показал `inactive`, а `NRestarts` — `0`. Юнит `Type=oneshot`/`idle`, поэтому `inactive` после `enabled` — норма (он уже отработал при загрузке), но это надо подтвердить: `systemctl cat balloo.service | grep -E 'Type|ExecStart'` + `systemctl status balloo.service --no-pager | head -12`. Реальный критерий — «после ребута стек поднимется сам» — проверяется только финальным ребутом.
