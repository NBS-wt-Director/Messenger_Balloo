...
> Создан: 2026-09-12. Последнее обновление: **2026-09-17, этап 32** (батч M на сервере: `login=401` — пароль `e2e_smtp_test` неизвестен; по решению пользователя создан **батч N — `docker/prod/ws-positive-test-auto.sh`**: скрипт сам генерирует пароль, пишет bcrypt-хеш (bcryptjs, cost 12, как в приложении) в БД тестовому аккаунту через psql и прогоняет весь тест без промптов; полностью проверен локально: psql-UPDATE на scratch-таблицах + полный прогон на стенде `PASS=5 FAIL=1` (FAIL ожидаем — мок ≠ dev-контейнер); next — commit+push, прогон батча N, UptimeRobot, P7/P12, финал/ребут).
...
## 1. ТЕКУЩАЯ ТОЧКА (2026-09-17, этап 32 — батч M на сервере дал `login=401` (пароль `e2e_smtp_test` неизвестен, ввод в промпт пользователь делать не может); создан батч N — `docker/prod/ws-positive-test-auto.sh`: пароль генерируется скриптом, bcrypt-хеш пишется в БД через psql, тест без промптов; next — commit+push, прогон батча N, UptimeRobot, P7/P12, финал/ребут)

### 1.0 Что делать в этой сессии прямо сейчас

1. **Текущее действие — батч N (§11):** после commit+push на сервере одной командой `cd ~/balloo && git pull --ff-only origin main && bash docker/prod/ws-positive-test-auto.sh`. Скрипт без промптов: сам генерирует пароль (`openssl rand`, только `A-Za-z0-9`), сам вычисляет bcrypt-хеш внутри контейнера `balloo-server` (bcryptjs, cost 12 — как в `authService.ts`; пароль передаётся по stdin, не в argv/историю), сам прописывает хеш аккаунту `o8eryuhtin@yandex.ru` (`UPDATE users SET "passwordHash"=…, "status"='active'` + выключение 2FA, `ON_ERROR_STOP`), сохраняет пароль в `~/balloo-ws-test-credentials.txt` (`600`, в stdout не печатается) и прогоняет тест батча M: login → ws-token → WS `101` → лог → restarts → P3 upload 5 МБ. Docker — через хелпер (без sudo, если хватает прав; иначе сам добавит `sudo`).
2. **Разбор прогона батча M (2026-09-17, этап 32):** скрипт сработал как задуман — промпты дошли (не как в пачке §1.5), но `login_http=401 «Неверный email или пароль»` — пароль аккаунта `e2e_smtp_test` неизвестен/не подходит. Прод исправен, дефекта нет; по решению пользователя («вводить без отображения не могу») пароль задаётся скриптом в БД (батч N). Вводить пароль в чат запрещено (правило §1.0.1) — скрипт решает проблему без этого.
...
6. **CANON (шаг 4) — `✅`** закрыт этапом 30: оба файла `e4a0f64d36062ccf…`, байт в байт.
...
5. **P9 unhandledRejection** — `✅` ЗАКРЫТА ПОЛНОСТЬЮ (этапы 28–31): деплой подтверждён этапом 30 (`process_handlers_js=1`, `index_registers=1`, `health_local=200`, `ws_no_token=401`), незакрытый замер «рестарты после `--force-recreate`» закрыт выводом пользователя 2026-09-17: `status=running restarts=0 startedAt=2026-09-17T04:41:29Z`.
6. **P7** — `~/.bashrc`, история, права на `metrics.sh` — не проверялось.

**Правим сами, решения не требуется:**

7. **AUTOSTART** — `✅` закрыт батчем F (2026-09-16): `is-enabled=enabled` (замер «до» — `disabled`). Остаток: проверка «поднимется ли стек сам» — только финальный ребут.
8. **P5** — `✅` закрыт 2026-09-16: `certbot renew --dry-run` → `success` по всем 7 сертификатам, таймер `enabled`.
9. **P3 поведение** — открыто: лимит `120m` в живом конфиге подтверждён (и sha256-равенством репо, этап 30), поведение на загрузке >1 МБ не проверено.
10. **A1 / P12** — `alpha.balloo.su=502` (чужое приложение). Базовой линии «как было» нет; устанавливаем причинно-следственную связь чтением чужого конфига, **не изменяя его** (правило 5). Замеры — батч C п.1.
11. **WS `101`** — `❓` единственная непроверенная позиция критериев: позитивный тест с настоящим токеном (§1.5) не выполнен.

### 1.3 Блокеров нет

Ни одна открытая позиция не мешает выкатке. Закрыто: P1, P2, P4, P5, P8(негатив), P9, P10, P11, P14, P15, P17(ротация probe), AUTOSTART-флаг, PROBES, CANON. Остаются: WS `101`, тест загрузки 413 (P3), UptimeRobot (пользователь), P7, P12 (чужая alpha), чистка истории/`.bashrc`, финальный ребут.

### 1.4 Ближайшее действие

**Батч N — скрипт `docker/prod/ws-positive-test-auto.sh`** (§11): после commit+push на сервере одной командой `cd ~/balloo && git pull --ff-only origin main && bash docker/prod/ws-positive-test-auto.sh`. **Промптов нет вообще** — скрипт сам генерирует пароль, пишет bcrypt-хеш в БД аккаунта `o8eryuhtin@yandex.ru` и прогоняет весь тест. Закрывает WS `101` (§10) + P3 (upload 5 МБ) + `[WS] Connection from user` + `restarts`. Батч M (`ws-positive-test.sh` с промптами) остаётся как альтернатива, если пароль известен. Дальше: UptimeRobot (пользователь, без сервера) → P7/P12 → финал с ребутом по вашей команде.

Выполненные батчи, не повторять: A (шаги 1–3), B, E, F, G, H, I, J, K, L, M (прогон — `login=401`, пароль неизвестен). Батч C (read-only) остаётся полезным для P12/`alpha.balloo.su` — его можно выполнить одним заходом вместе с P7.
...
| P3 | Нет `client_max_body_size` в живом конфиге → 413 на >1 МБ | `🟡` `client_max_body_size 120m` **в живом конфиге есть** (строка 14) и конфиг = репо байт в байт (этап 30). Поведенческий тест (upload 5 МБ) — включён в батч M (§11): PASS = любой код кроме `413`/`401` (ожидаемо `400 «chatId обязателен»` — тело дошло до приложения) | Прогон батча M |
...
| P8 | WS-путь ронял весь процесс API | `✅` **ЗАКРЫТ 2026-09-14** (`401`/`401`, `restarts=0`, 0 лишних инициализаций). Позитив `101` — батч M (§11), скрипт готов и проверен локально | Прогон батча M |
...
- [ ] Загрузка файла 5–50 МБ проходит без 413 — `❓` P3: лимит `120m` в живом конфиге подтверждён; поведенческий тест — батч M (§11), PASS = код ≠ `413`/`401` (ожидаемо `400 «chatId обязателен»`)
- [ ] WS: `wss://balloo.su/ws/?token=<access>` отвечает `101`, без токена `401`, процесс не падает — `🟡` P8: `401` и «не падает» подтверждены 2026-09-14; `101` — батч M (§11, скрипт готов и проверен локально; `rc=28` при `101` — норма, критерий — код)
...
### Батч N — автоматический WS-тест с записью пароля в БД — ТЕКУЩЕЕ ДЕЙСТВИЕ (этап 32)
