...
2. **P11 SMTP — 🟡 релей готов (этап 25: баннер `220` из контейнера, `mynetworks` дополнен, ufw-правило для docker-подсети есть). Осталось применить env и проверить письмо (батч J):** на сервере `cd ~/balloo && git pull --ff-only origin main` (в репо: `SMTP_HOST=172.19.0.1`, `SMTP_PORT=25`, `SMTP_USER`/`SMTP_PASSWORD` убраны — postfix без SASL, nodemailer с заданным auth падает) → `sudo ufw allow 25/tcp` (25 наружу — пользователь подтвердил «открыть», для входящей почты @balloo.su) → пересоздание контейнера → проверка env в контейнере → тестовое письмо. ⛔ Инцидент этапа 25: `SMTP_PASSWORD` показан в чате моей неверной маской (§1.0.1) — ротация §8 обязательна, план не меняется (он уже был в списке).
...
| P11 | SMTP не настроен | `🟡` **батч G (этап 23): на хосте уже стоит postfix+dovecot** (enabled, `:25`), MX=`mail.balloo.su`, SPF уже разрешает наш IP, исходящий `:25` открыт, docker-сеть `prod_balloo-net` (шлюз `172.19.0.1`). Схема: приложение → хостовый postfix (`SMTP_HOST=172.19.0.1:25`), без нового контейнера. `SMTP_USER`/`SMTP_PASS`/`SMTP_TLS` при этой схеме не нужны | Батч H (read-only): `postconf myhostname/mydestination/mynetworks/relayhost`, `mail.balloo.su` → этот ли сервер, логи postfix, баннер из контейнера, ufw:25 → подтверждение пользователя «постфикс наш» → правка `mynetworks` (+`172.19.0.0/16`) одним заходом → env → тест письма из контейнера. Отдельно (не блокер): rDNS у провайдера planeta.tc, DMARC-запись |
