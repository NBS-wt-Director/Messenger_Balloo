...
2. **P11 SMTP — 🟡 применено и verified (этап 26)**: релей + env + `[EMAIL] SMTP connection verified successfully`. Остался **e2e-тест письмом** (последний признак закрытия P11): инициировать отправку из UI (регистрация нового аккаунта — только по явному «да» пользователя, §1.5, либо сброс пароля существующего) и увидеть в `/var/log/mail.log` `from=<noreply@balloo.su>` → `status=sent`. После — ротация §8, WS `101`, финал/ребут.
...
| P11 | SMTP не настроен | `🟡` **применено и verified (этап 26)**: postfix доступен из контейнера (баннер 220), env применён (`172.19.0.1:25`, без auth), ufw 25 открыт наружу (входящая почта), в логах приложения `[EMAIL] SMTP connection verified successfully`. Постфикс наш, доставляет напрямую, SPF уже разрешал наш IP | Последний признак закрытия — e2e-письмо: инициировать отправку (регистрация/сброс пароля из UI) и увидеть в `/var/log/mail.log` `from=<noreply@balloo.su>` → `status=sent`. Отдельно (не блокер P11): rDNS у провайдера planeta.tc, DKIM/DMARC — после базовой схемы |
