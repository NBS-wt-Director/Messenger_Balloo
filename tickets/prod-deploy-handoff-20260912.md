...
> Создан: 2026-09-12. Последнее обновление: **2026-09-16, этап 19** (разобран реальный вывод батча F: падал не `tee`, а редирект `wc -l < "$HTP"`; скрипт переписан, добавлена локальная самопроверка → повтор батча F одной командой).
...
## 1. ТЕКУЩАЯ ТОЧКА (2026-09-16, этап 19 — причина падения батча F найдена по исходнику и воспроизведена локально; скрипт переписан, есть зелёная самопроверка → повтор батча F одной командой)

### 1.0 Что делать в этой сессии прямо сейчас

1. **Единственное подтверждённое состояние сервера на сегодня** — вывод пользователя от 2026-09-15/16: прод жив (`balloo-server Up 27 hours (healthy)`, `balloo-web Up 4 days`, `balloo-postgres Up 4 days (healthy)`, `balloo-redis Up 5 days (healthy)`, `balloo-minio Up 5 days (healthy)`), репозиторий `~/balloo` на `9c16d23` (он же `origin/main`), запуск `bash docker/prod/enable-probes.sh` дошёл до `docker/prod/enable-probes.sh: line 66: /etc/nginx/.htpasswd-balloo: Permission denied` и остановился. Всё остальное в §1 — либо из более раннего вывода, либо помечено `❓`/`⛔`.
2. **Реальная причина падения — не `tee`, а редирект** (разбор по исходнику `9c16d23` + локальное воспроизведение, §1.0.0 строка «диагноз»). Строка 66 там такая: `LINES=$($SUDO wc -l < "$HTP" | tr -d '[:space:]')`. Кавычки `< "$HTP"` открывает **непривилегированная** оболочка до запуска `sudo`, а файл к этому моменту уже `640 root:www-data`. `tee`/`chown`/`chmod` (строки 62–64) выполнены **с sudo и успешно** — значит **пароль, введённый в промпт, записан в `/etc/nginx/.htpasswd-balloo`**. Конфиг nginx не тронут, `reload` не выполнялся, `systemctl enable` не запускался.
3. **Что уже сделано в этом этапе (репозиторий, `🟡` — на сервере не подтверждено):** `docker/prod/enable-probes.sh` переписан целиком (282 строки вместо 120) и покрыт локальной самопроверкой `docker/prod/enable-probes.selftest.sh` — **40 PASS / 0 FAIL**. Ключевые отличия от версии `9c16d23`: ни одного shell-редиректа на root-файл; пароль **не запрашивается и не печатается** (генерируется `openssl rand`, кладётся в `~/balloo-probe-credentials.txt` `chmod 600`); gate идёт **до** всяких записей и показывает `diff` живого конфига с каноном + спрашивает `y`; проверяется владелец/права `htpasswd` и то, что файл **читает пользователь воркеров nginx**; проверяется, что путь в `auth_basic_user_file` канона совпадает с путём, куда пишется пароль; после `reload` проверяется `nginx -T` и коды `401/200`; при провале печатается `tail error.log`; идемпотентен.
4. **Дальше один шаг — повтор батча F.** Команда (одна строка, на сервере, пользователь `cfr_balloo`):
   ```bash
   cd ~/balloo && git pull --ff-only origin main && bash docker/prod/enable-probes.sh
   ```
   Ждём в выводе: `HEAD` = новый коммит (не `9c16d23`), `htpasswd` до/после (одна строка, `$6$…`), `воркер nginx (www-data) читает …`, `OK в загруженной конфигурации nginx 2 блока`, три строки `PASS … без пароля=401 с паролем=200`, `ИТОГ: проверок провалено = 0`. Промпт `применить канон …? [y/N]` — читать diff выше и отвечать `y` только если там **только** добавление `auth_basic`/`auth_basic_user_file`. Если в diff есть чужие/лишние строки — ответить `n` (ни один файл не изменится) и прислать вывод целиком.
   **Пароль в чат не присылать.** Скрипт кладёт его в `~/balloo-probe-credentials.txt` (`chmod 600`) и в stdout его не печатает — вывод можно слать целиком, секретных строк в нём нет.
5. **Аннулировано как выдумка предыдущих сессий (не возвращать):** эндпоинты `/livez`, `/readyz`, `/api/livez`; файл `/etc/nginx/.htpasswd-probe`; скрипт `fix-auth-probes.sh`; «пустой хеш в htpasswd — источник 401». Реальный `401` на `127.0.0.1:8080` даёт **чужой node** (батч B), к нашему стеку отношения не имеет. Наши пробы — `/health`, `/health/ready` (§1.0.4, §3).
6. **Миграционный блок предыдущего сообщения — ложный дрейф, отозван.** Pending-миграций нет (этап 1 завершён: `tables=51`, «up to date»). **Данные не тронуты, откатывать нечего.**
7. **Ничего не выдумывать:** каждый путь, имя юнита и утилита в команде — либо из §3, либо из §1.1/§1.6/журнала этого документа, либо из вывода, присланного пользователем.
...
| P14 | **Что лежит в `/etc/nginx/.htpasswd-balloo`.** История: пачка этапа 15 не могла задать пароль (`read: «PW?…» not a valid identifier` → `PW` пустая, `htpasswd: command not found`), поэтому опасались хеша пустой строки. Этап 19: батч F от `9c16d23` отработал строки `sudo tee` → `chown root:www-data` → `chmod 640` и упал **после** них на редиректе `wc -l < "$HTP"` → в файле лежит `uptimerobot:<sha512-crypt введённого в промпт пароля>` | `🟡` вывод не получен, но вывод следует из исходника `9c16d23` и формата ошибки сервера (см. журнал стр. 19). Пустого хеша там быть не может | Лечится автоматом: повтор батча F перезаписывает файл свежим паролем и печатает замеры `htpasswd` до/после (`пользователь=… хеш=$6$… длина=…`, значение хеша не печатается). Отдельной команды не нужно |
| P15 | **Дошёл ли канон до живого конфига.** Подтверждённого замера после `c32f81c` нет: вывод этапа 17 обрывается до `cp`/`nginx -t`/`reload`, а строки журнала 16–18 противоречат друг другу (пометка после §2) | `❓` живой конфиг не сверялся с `c32f81c`. Замер `e4a0f64d36062ccfa5a6ca8ec4990d08b909320addc4ff8a7074b7efd7fafca7` = `docker/prod/nginx/balloo-docker.conf` в `HEAD` (измерено локально, `sha256sum`), он же канон с двумя `auth_basic`-блоками | Замеряет сам скрипт перед любыми действиями: `sha256 живого`, `sha256 канона`, число блоков в живом, `diff -u` живого с каноном, а после `reload` — число наших блоков в `nginx -T`. Отдельная команда не нужна |
| P17 | **Пароль от probe-доступа введён пользователем в чат** (2026-09-15, этап 17). Значение намеренно НЕ переносится в этот документ. Второй однотипный инцидент после `MAILRU_CLIENT_SECRET` (§1.7). Уточнение этапа 19: в отличие от ранней записи, этот пароль **успел попасть в `/etc/nginx/.htpasswd-balloo`** (P14), то есть он не «нигде не используется», а действующий credential probe-пользователя | `❓` требует ротации | (а) повтор батча F по умолчанию генерирует новый пароль и перезаписывает `htpasswd` — засвеченное значение перестаёт действовать сразу после прогона; новый пароль лежит в `~/balloo-probe-credentials.txt` (`600`) и в stdout не печатается, поэтому вывод скрипта можно слать в чат целиком; (б) смена sudo-пароля `cfr_balloo` — отдельный шаг по явной команде пользователя (`passwd` на сервере), не откладывать бессрочно; (в) правило: ассистент не принимает пароли в переписке и не просит их придумывать (§1.0.1) |
...
- [ ] Probe-эндпоинты закрыты basic-auth (401 снаружи), healthcheck'и контейнеров `healthy` — `🟡` канон и скрипт готовы, самопроверка зелёная; на сервере не применено. Healthcheck'и `healthy` подтверждены (батч B, `docker ps` 2026-09-16). Признак закрытия: `ИТОГ: проверок провалено = 0` и три `PASS … без пароля=401 с паролем=200` в выводе батча F
- [ ] `certbot renew --dry-run` зелёный, таймер активен — `❓` P5 (сертификаты подтверждены: `balloo.su`, `api.balloo.su`; dry-run не запускался)
- [ ] `balloo.service` в `enabled`, после ребута стек поднимается без ручных команд — `❓` статус флага не подтверждён (противоречие строк 16/18, пометка после §2); включает и печатает статус батч F. Проверка «после ребута» — только финальным ребутом
