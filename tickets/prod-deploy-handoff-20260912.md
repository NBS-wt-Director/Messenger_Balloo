...
> Создан: 2026-09-12. Последнее обновление: **2026-09-17, этап 33** (**батч N на сервере: `ИТОГ: PASS=6 FAIL=0`** — WS `101` подтверждён с настоящим токеном (`ws_with_token=101`, `ws_accepted_log=1`, `restarts=0`), **P3 закрыт** (upload 5 МБ → `400 «chatId обязателен»`, `413` нет); пароль тестового аккаунта — в `~/balloo-ws-test-credentials.txt` (`600`); из критериев §10 остаются: UptimeRobot (пользователь), P12 (чужая alpha), автозапуск после ребута; next — UptimeRobot → P7/P12 (батч C) → финал/ребут).
...
2. **Дальше по порядку:** (а) **UptimeRobot — пользователь, без сервера**: монитор «HTTP(s)», URL `https://api.balloo.su/health`, GET, 60 с, Basic Auth `uptimerobot` + пароль из `~/balloo-probe-credentials.txt` (`cat` на сервере, в чат не переносить); (б) **P7/P12 — батч C (§11, read-only)** одним заходом; (в) **финал (§6 шаг 8)**: `ufw status numbered` → `certbot renew --dry-run` (уже зелёный 2026-09-16, повтор по желанию) → починка `.bashrc`/истории (P7) → **ребут по команде пользователя** → `docker compose ps` без команд → smoke-тест.
...
1. **UptimeRobot** — `❓`: настроить монитор «HTTP(s)», URL `https://api.balloo.su/health`, GET, 60 с, Basic Auth `uptimerobot` + пароль из `~/balloo-probe-credentials.txt` (в чат не переносить). Подтвердить настройку.
...
Ни одна открытая позиция не мешает выкатке. Закрыто: P1, P2, P3, P4, P5, P8, P9, P10, P11, P14, P15, P17(ротация probe), AUTOSTART-флаг, PROBES, CANON, **WS `101`, P3-поведение** (батч N, этап 33). Остаются: UptimeRobot (пользователь), P7, P12 (чужая alpha), чистка истории/`.bashrc`, судьба тестовых аккаунтов (`e2e_smtp_test`, `probe@balloo.su`), финальный ребут.

### 1.4 Ближайшее действие

1. **UptimeRobot — пользователь, без сервера** (можно параллельно со всем остальным): монитор «HTTP(s)», URL `https://api.balloo.su/health`, GET, 60 с, Basic Auth `uptimerobot` + пароль из `~/balloo-probe-credentials.txt` (`cat` на сервере, в чат не переносить). После настройки подтвердить — закрывает мониторинг.
