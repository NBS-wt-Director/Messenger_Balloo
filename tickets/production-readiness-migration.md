...
> **Статус:** ✅ Тикет №1 готов. Ожидание apikeys.json (пользователь заполнит позже).
...
## ⚡ Блокировка до apikeys.json

**Все тикеты после №1 заблокированы** — требуется заполненный `apikeys.json` для генерации `.env.dev` и `.env.production`.

| Тикет | Почему заблокирован |
|-------|---------------------|
| №3 (Prisma) | Нужен `DATABASE_URL` из .env |
| №4 (Server API) | Нужны JWT/OAuth/Redis/MinIO ключи |
| №5 (Web) | Нужны API-ключи для бэкенда |
| №6–8 (Desktop/Mobile) | Зависят от Web |
| №9 (Тесты) | Зависит от Server + Web |
| №10 (Docker) | Нужны все ключи для сервисов |
| №11 (CI/CD) | Зависит от тестов |
| №12 (Документация) | Зависит от всего |

**Как разблокировать:** пришлите заполненный `apikeys.json` — я генерирую `.env.dev` + `.env.production` и запускаю №2→№3→№4...
...
### Фаза 0 — Блокировка (текущая)

**Ожидание apikeys.json от пользователя.** Без ключей невозможно запустить ни один тикет после №1.

### Фаза 1 — Разблокировка и бэкенд

После получения apikeys.json запускаем последовательно:

```
№2 → №3 → №4 → №5 → №10
```

- №2 — генерация .env.dev + .env.production
- №3 — Prisma миграции (требует DATABASE_URL из №2)
- №4 — Server API (требует ключи из №2)
- №5 — Web (требует API-ключи из №2)
- №10 — Docker (требует ключи из №2, можно параллельно с №4)
...
### Фаза 3 — Тесты и финализация

```
№9 (Тесты) → №11 (CI/CD) → №12 (Документация)
```
