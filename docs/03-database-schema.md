...
## 🆕 Дисциплина миграций: альфа + прод на ЕДИНОЙ БД (решение владельца 2026-09-19)

**Альфа-окружение (testing) работает на той же БД, что и прод** (те же postgres/redis/minio контейнеры). Из этого следуют **жёсткие правила для любой миграции схемы**:

1. **Только обратно-совместимые миграции.** Пока на альфе крутится новый код, на проде продолжает работать старый — оба должны работать на одной схеме одновременно.
2. **Разрешено (additive-only):** новые таблицы; новые колонки **с `@default` или nullable**; новые значения enum; новые индексы (не `UNIQUE`, ломающий старые данные); новые связи.
3. **Запрещено (destructive/breaking):** `DROP TABLE`/`DROP COLUMN`; переименование таблиц/колонок; смена типов; удаление/обнуление данных; `UNIQUE`-констрейнты, под которые старые данные не подходят; удаление enum-значений, которые старый код ещё пишет.
4. **Канал миграций один — Prisma migrate** (`migrate deploy`); `prisma db push` запрещён (правило с этапа 1 деплой-тикета).
5. **Перед применением миграции на общей БД:** (а) SQL миграции читается глазами на предмет п.2–3; (б) свежий бэкап БД (`pg_dump` в MinIO/файл); (в) миграция деплоится сначала на альфу и живёт там ≥ одного цикла приёмки, только затем — на прод.
6. **Откат миграции назад не проектируется** — поэтому правило одно: не добавлять в схему ничего, что нельзя оставить навсегда. Если фича не взлетела — её таблицы остаются пустыми (soft-archived), чистка — только отдельным решением владельца после остановки прода.
**Заявление владельца (2026-09-19): поля пользователей (`users`/`oAuthAccounts`/`public_profiles` и связанные) будут дополняться и дальше** — это штатный режим развития схемы при альфа+прод на единой БД. Правила п.2–3 выше покрывают его полностью: новые колонки только nullable или с `@default`; старый прод-код игнорирует новые поля, новый альфа-код использует — оба работают на одной схеме одновременно. Удалять структуру и данные нельзя — только дополнять.

7. Удаление/анонимизация данных (аккаунты, сообщения) — только штатными путями приложения (soft-delete), не SQL-командами поверх прод-БД.
