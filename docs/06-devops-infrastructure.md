...
## 🆕 Альфа-окружение (testing) — решение владельца 2026-09-19

**Проблема:** сейчас деплой идёт напрямую «дев с ноута → прод на сервере» — непроверенная фича (глючная/лишняя) сразу попадает к пользователям.

**Целевой конвейер:** **дев с ноута → альфа (testing) на сервере → прод на сервере.** Новая фича сначала выкатывается на альфа-поддомен и проверяется особой подгруппой тестировщиков; на прод льётся только после приёмки.

### Что входит в альфу — и что НЕ входит

| Выводится в альфу | НЕ выводится (общие с продом) |
|---|---|
| Сборки `server` (контейнер `balloo-server-alpha`) | **БД PostgreSQL** — единая с продом |
| Сборки `web` (контейнер `balloo-web-alpha`) | Redis, MinIO |
| Сборки `admin` | nginx (общий процесс, свой vhost), docker-сеть `prod_balloo-net` |
| | Статические сайты (history/download/docs/blog) |

### Следствия единой БД (жёсткие требования)

1. **Миграции — только обратно-совместимые, additive-only** (подробно: `docs/03-database-schema.md`, раздел «Дисциплина миграций»). Пока альфа крутит новый код, прод крутит старый — оба на одной схеме.
2. **Не удалять структуру и данные — только дополнять**: новые таблицы/колонки (nullable или с default), новые enum-значения. Запрещены DROP/RENAME/смена типов/удаление данных.
3. Перед любой миграцией на общей БД: SQL читается глазами + свежий бэкап (`pg_dump`); сначала деплой на альфу, после цикла приёмки — на прод.
4. Тестовые данные, создаваемые тестировщиками на альфе, попадают в общую БД — тестировщики работают с осознанием, что это прод-данные (аккаунты/чаты тестировщиков помечаются как тестовые).

### Компоненты альфы

- **Поддомен: `alpha.balloo.su`** (решение владельца 2026-09-19: вся зона `balloo.su` под владельцем). DNS уже указывает на сервер; сертификат уже выпущен (в списке certbot). При старте — снять мёртвый vhost (upstream `localhost:3021`) и наложить канон `docker/prod/nginx/alpha.balloo.su.conf` откатывающимся скриптом.
- **Compose:** отдельный файл/профиль (`docker-compose.alpha.yml` или `--profile alpha`): сервисы `balloo-server-alpha`/`balloo-web-alpha` на своих портах `127.0.0.1` (3200/8091), сеть `prod_balloo-net`, те же postgres/redis/minio (новые контейнеры БД НЕ создаются).
- **Env:** `.env.alpha` — копия прод-параметров + `VITE_API_URL` (зависит от режима, см. ниже) + `CORS_ORIGIN` со списком доменов (прод + альфа).

### Единая авторизация альфа+прод (решение владельца 2026-09-19)

Код авторизации один (различия — только env); аккаунты одни (единая БД: `users`/`oAuthAccounts` общие, вход через провайдера матчится по `provider+providerId` — один пользователь на альфе и проде; сессии раздельные, cookie host-only на своём api-домене).

Два режима альфы (выбирается env на выкате):

| Режим | Что тестируем | API alpha-web | OAuth у провайдеров |
|---|---|---|---|
| **Фронтовая альфа** | только фронт | **прод-API** `api.balloo.su` | без изменений (авторизация полностью общая) |
| **Серверная альфа** | серверный код | свой `balloo-server-alpha` (альфа-домен) | VK/Mail.ru: добавить альфа-redirect_uri в те же приложения; Яндекс: проверить множественность Callback URL, иначе второе приложение «Balloo Alpha» или OAuth-тесты только на проде |

Поддомены `*.balloo.su` — same-site: host-only cookie на api-домене работает для всех страниц зоны (fetch `credentials: 'include'`, `CORS_ORIGIN` — список).

### Порядок выката фичи (конвейер)

ноут push → сервер pull → `build` альфа-образов → `up -d` альфа-сервисов → приёмка подгруппой тестировщиков → тот же код поднимается на проде (`build web server` + `up -d web server`) → приёмка продом. При фронтовой альфе серверная часть не пересобирается вовсе.

**Старт подзадачи:** после полного деплоя регистрации/авторизации (по команде владельца). Полный план и чек-лист — `tickets/prod-deploy-handoff-20260912.md`, §6-квинке.
