...
- **Поддомен:** ⚠️ `alpha.balloo.su` занят чужим vhost (мёртвый upstream 3021) — имя выбрать другое (`testing.balloo.su` / `dev.balloo.su`) или освободать `alpha.*` (чужое — по согласованию).
- **DNS:** A-запись поддомена → 188.73.176.34 (действие владельца у регистратора/DNS).
- **Сертификат:** `certbot --nginx -d <поддомен>` (одна команда владельца; authenticator = nginx, по правилам деплой-тикета).
- **nginx:** канонический vhost в репо (`docker/prod/nginx/`), наложение — откатывающим самопроверяющимся скриптом (бэкап → gate → `nginx -t` → один `reload`).
- **Compose:** отдельный файл/профиль (`docker-compose.alpha.yml` или `--profile alpha`): сервисы `balloo-server-alpha`/`balloo-web-alpha` на своих портах `127.0.0.1` (например 3200/8091), сеть `prod_balloo-net`, те же postgres/redis/minio (новые контейнеры БД НЕ создаются).
- **Env:** `.env.alpha` — копия прод-параметров + `VITE_API_URL=https://<альфа-поддомен>` (запекается в бандль альфа-web), `CORS_ORIGIN` альфа-домена.
- **OAuth на альфе:** redirect_uri альфа-домена должен быть прописан в панелях провайдеров (у Яндекса Callback URL один на приложение → либо отдельное «альфа»-приложение у каждого провайдера, либо OAuth-тесты только на проде; решение владельца).
- **Порядок выката фичи:** ноут push → сервер pull → `build` альфа-образов → `up -d` альфа-сервисов → приёмка подгруппой → тот же образ/код поднимается на проде (`build web server` + `up -d web server`) → приёмка продом.
